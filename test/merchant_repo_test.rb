...
class MerchantRepositoryTest < Minitest::Test
  ...
  def test_returns_all_matches_of_merchant
    mr = MerchantRepository.new
    mr.load_data("./data/bogus_merchants.csv")
    merchant1 = mr.all_by_name("miniaturebikez")

    assert_equal "MiniatureBikez", merchant1[0].name
    assert_equal "12334113", merchant1[0].id
    assert_equal "MiniatureBikez", merchant1[1].name
  end
end
